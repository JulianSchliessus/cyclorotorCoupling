...
class MBDynInput:
    ...
    def __init__(self):
        ...
        self.design_variables = DesignVariablesBlock()
    ...
    def create_from_prep(self, prep):
        ...
        self.set_design_variable_block(prep.design_variable_dict)
    ...
    def set_design_variable_block(self, design_variable_dict):
        #self.count_cards_by_type()
        for key, value in design_variable_dict.items():
            self.design_variables.variables[key] = float(value)               
    ...
    def write_input_file(self, file_name=None):
        ...
        with open(self._file + '.mbd', 'w') as input_file:
            ...
            input_file.write(self.design_variables.get_block_str())
...
class Block:
    ...
    def get_block_str(self):
        ...
        if self.name == '':
        	var_block = True
        	self.name = self.name_var_block
        
        if var_block:
        	block_str = self._block_sep_str()
        else:
...
class ProblemBlock(Block):
    def __init__(self):
        ...
        self.entries['tolerance'] = 1e-6
        self.entries['max iterations'] = 10
...
class  DesignVariablesBlock(Block):
    def __init__(self):
        super().__init__()
        #self.name = 'control data'
        self.name = ''
        self.name_var_block = 'Design Variables'
        self.variables = dict()
        ...
        if len(self.variables) > 0:
            ...
            for var_label, var_value in self.variables.items():
                ...
                block_str += 'set: {nset} {lbl}{spc} = {val}{lend}'.format(
                    ...
                    spc=(11-len(var_label)) * ' ', val=var_value,
                    lend=self.line_end_str)
        block_str += 'set: real theta1 = pi/6.; # Initial angle of Link1 w.r.t vertical line{lend}'.format(lend=self.line_end_str)
        block_str += 'set: real theta2 = 7.*pi/12.; # Initial angle of Link2 w.r.t Link1{lend}'.format(lend=self.line_end_str)
        return block_str
...
class ElementsBlock(Block):
    ...
    def bodies_from_mesh(self, input_mesh, indexing):
        ...
        for i, node_area in enumerate(node_areas):
            ...
            self.create_body(index_offset+i, i, '1',
                             'null', 'eye')
